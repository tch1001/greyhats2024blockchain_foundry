...
contract VotingVault {
    ...
    function unlock(uint256 end) external returns (uint256 amount) {
        ...
        require(
            block.timestamp > depositToUnlock.unlockTimestamp,
            "still locked"
        );

        amount =
            depositToUnlock.cumulativeAmount -
            lastUnlockedDeposit.cumulativeAmount;
        data.front = end;
    ...
    function delegate(address newDelegatee) external {
        ...
        uint256 amount = lastDeposit.cumulativeAmount -
            lastUnlockedDeposit.cumulativeAmount;
