...
contract DualAssetEscrow is IEscrow, Clone {
    ...
    function initialize() external {
        ...
        - 4 bytes for selector              fn
        - 20 bytes for factory address      args
        - 20 bytes for tokenX address
        - 20 bytes for tokenY address
        - 2 bytes for CWIA length           length (part of args)
        // abcd(0004)
        */
        ...
        escrowId = uint256(
            keccak256(abi.encodePacked(IDENTIFIER, factory, tokenX, tokenY))
        );
    }
    ...
    function _getArgs()
        internal
        pure
        returns (address factory, address tokenX, address tokenY)
    {
